  ...
  $.getJSON("api/stream/" + b64json + from, null, function(data) {
    if (data != null) {
      ...
      for (var obj in data.hits.hits) {
        ...
        index = hit['_index']
        if (!(has_time)) {
          window.last_time = get_field_value(hit,data.kibana.timestamp_field);
          has_time = true;
        }
        if ($('#logrow_' + id).length == 0) {
          ...
          linkTableData.text(prettyDateString(Date.parse(get_field_value(hit,data.kibana.timestamp_field)) + tOffset)).prepend(jlink);
