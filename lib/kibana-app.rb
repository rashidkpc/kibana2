...
class KibanaApp < Sinatra::Base
  ...
  get '/api/analyze/:field/trend/:hash' do
    ...
    query_end     = SortedQuery.new(
      req.search,req.from,req.to,0,limit,KibanaConfig::Time_field,'desc')
    ...
    query_begin   = SortedQuery.new(
      req.search,req.from,req.to,0,limit,KibanaConfig::Time_field,'asc')
  ...
  get '/rss/:hash/?:count?' do
    ...
    content = RSS::Maker.make('2.0') do |m|
      ...
      result.response['hits']['hits'].each do |hit|
        ...
        i.date  = Time.iso8601(KelasticResponse.get_field_value(hit,KibanaConfig::Time_field))
        i.link  = link_to("/##{hash}")
