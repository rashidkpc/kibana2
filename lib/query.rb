...
class SortedQuery < Query
  attr_accessor :query,:from,:to
  def initialize(question, from, to, offset = 0, size = KibanaConfig::Per_page, field = KibanaConfig::Time_field, order = "desc")
...
class DateHistogram < Query
  def initialize(question, from, to, interval, field = KibanaConfig::Time_field)
    super(question, from, to)
    @query['facets'] = {
      "count" => {
        "date_histogram" => {
...
=end
class StatsHistogram < Query
  def initialize(question, from, to, field, interval, key_field = KibanaConfig::Time_field)
    super(question, from, to)
    @query['facets'] = {
      "mean" => {
        "date_histogram" => {
